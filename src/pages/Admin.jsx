        ...
        <div>
          {tab === 'dashboard' && (
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginBottom: '18px' }}>
                {[
                  { label: 'Total Users', value: users.length.toLocaleString(), color: '#8b5cf6', delta: '+12 this week' },
                  ...
                  { label: 'Daily Active', value: '2,840', color: '#06b6d4', delta: '+8% 7-day avg' },
                ].map(s => (
                  <div key={s.label} style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '14px' }}>
                    ...
                    <div style={{ color: '#94a3b8', fontSize: '11px', marginTop: '2px' }}>{s.delta}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '16px' }}>
                  ...
                  {[
                    { name: 'Sponsored Listings', amount: 8420, color: '#8b5cf6' },
                    { name: 'Facebook Ads (15%)', amount: 1248, color: '#06b6d4' },
                    ...
                    return (
                      <div key={src.name} style={{ marginBottom: '10px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                          <span style={{ color: '#94a3b8', fontSize: '12px' }}>{src.name}</span>
                ...
                <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '16px' }}>
                  ...
                  {[
                    ...
                    { action: 'Reviewed report #4821', time: '3h ago', color: '#06b6d4' },
                    { action: 'Processed refund', time: '5h ago', color: '#8b5cf6' },
                  ].map((a, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 0', borderBottom: i < 4 ? '1px solid #1e1e2e' : 'none' }}>
                      ...
                      <span style={{ color: '#475569', fontSize: '11px' }}>{a.time}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              ...
              <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                {deals.slice(0, 12).map(d => (
                  <div key={d.id} style={{ padding: '12px 14px', borderBottom: '1px solid #1e1e2e', display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ color: '#475569', fontSize: '11px' }}>{d.city}, {d.state} · ${(d.listingPrice || d.price).toLocaleString()} · {d.sellerName}</div>
                    </div>
                    {d.isSponsored && <span style={{ padding: '3px 8px', borderRadius: '12px', background: 'rgba(139,92,246,0.15)', color: '#8b5cf6', fontSize: '10px', fontWeight: 700 }}>SPONSORED</span>}
            ...
            <div>
              ...
              <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                {[
                  ...
                  <div key={i} style={{ padding: '14px 16px', borderBottom: '1px solid #1e1e2e' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
                      ...
                      <span style={{ color: '#475569', fontSize: '11px' }}>{dm.date}</span>
                    </div>
                    <div style={{ color: '#94a3b8', fontSize: '12px', fontStyle: 'italic', marginBottom: '8px' }}>"{dm.preview}"</div>
                ...
                {[
                  ...
                  <div key={i} style={{ padding: '14px 16px', borderBottom: '1px solid #1e1e2e' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                      ...
                      <span style={{ marginLeft: 'auto', color: '#475569', fontSize: '11px' }}>{p.likes} likes</span>
                    </div>
                    <p style={{ color: '#94a3b8', fontSize: '13px', margin: '0 0 10px', lineHeight: 1.5 }}>{p.content}</p>
                    <div style={{ display: 'flex', gap: '6px' }}>
            ...
            <div>
              ...
              <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                {[
                  ...
                  <div key={o.id} style={{ padding: '12px 14px', borderBottom: '1px solid #1e1e2e', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    <code style={{ color: '#06b6d4', fontSize: '11px' }}>{o.id}</code>
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#f8fafc', fontSize: '13px', fontWeight: 600 }}>{o.user}</div>
                      <div style={{ color: '#475569', fontSize: '11px' }}>{o.city} · {o.trades}</div>
                    </div>
                    <div style={{ color: '#10b981', fontWeight: 800, fontSize: '14px' }}>{o.amount}</div>
                    <div style={{ color: '#475569', fontSize: '11px' }}>{o.date}</div>
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                {[
                  ...
                  <div key={o.id} style={{ padding: '12px 14px', borderBottom: '1px solid #1e1e2e', display: 'flex', alignItems: 'center', gap: '14px' }}>
                    <code style={{ color: '#8b5cf6', fontSize: '11px' }}>{o.id}</code>
                    <div style={{ flex: 1 }}>
                      <div style={{ color: '#f8fafc', fontSize: '13px', fontWeight: 600 }}>{o.deal}</div>
                      <div style={{ color: '#475569', fontSize: '11px' }}>{o.user} · {o.tier} · {o.rate}</div>
                    </div>
            ...
            <div>
              ...
              <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                {[
                  ...
                  <div key={o.id} style={{ padding: '14px 16px', borderBottom: '1px solid #1e1e2e' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                      {[
                        ...
                        ['Impressions', o.impressions, '#06b6d4'],
                        ['Clicks', o.clicks, '#8b5cf6'],
                        ['Leads', o.leads, '#10b981'],
                      ].map(([l, v, c]) => (
                        <div key={l} style={{ background: '#1a1a2e', borderRadius: '8px', padding: '8px 10px' }}>
                          <div style={{ color: '#475569', fontSize: '10px', fontWeight: 700 }}>{l}</div>
              ...
              <div style={{ marginTop: '12px', padding: '14px', background: 'rgba(6,182,212,0.05)', border: '1px solid rgba(6,182,212,0.2)', borderRadius: '10px', fontSize: '13px', color: '#06b6d4' }}>
          ...
          {tab === 'banned' && (
            <div>
              ...
              {bannedIds.size === 0 ? (
                <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', padding: '40px', textAlign: 'center' }}>
                  <Ban size={32} style={{ color: '#475569', marginBottom: '10px' }} />
                  <div style={{ color: '#94a3b8', fontSize: '14px', fontWeight: 600 }}>No banned accounts</div>
                  <div style={{ color: '#475569', fontSize: '12px', marginTop: '4px' }}>Accounts you ban from the Users tab will appear here with IP ban info.</div>
                </div>
              ) : (
                <div style={{ background: '#12121e', border: '1px solid #1e1e2e', borderRadius: '12px', overflow: 'hidden' }}>
                  {Array.from(bannedIds).map(uid => {
                    ...
                    if (!u) return null;
                    return (
                      <div key={uid} style={{ padding: '12px 14px', borderBottom: '1px solid #1e1e2e', display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <img src={u.avatar} alt="" style={{ width: '36px', height: '36px', borderRadius: '50%', opacity: 0.4 }} />
                        <div style={{ flex: 1 }}>
                          <div style={{ color: '#f8fafc', fontSize: '13px', fontWeight: 600 }}>{u.name} <span style={{ color: '#475569' }}>@{u.username}</span></div>
